...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Atom Builder Functionality
document.addEventListener('DOMContentLoaded', function() {
    // Add event listener to Atom Builder box (if not already handled by existing code)
    const atomBuilderBox = document.getElementById('atom-builder-box');
    if (atomBuilderBox) {
        atomBuilderBox.addEventListener('click', function() {
            // Hide all detail views
            document.querySelectorAll('.detail-view').forEach(view => {
                view.style.display = 'none';
            });
            // Show atom builder view
            document.getElementById('atom-builder-view').style.display = 'block';
        });
    }
    
    // Elements
    const backBtn = document.querySelector('#atom-builder-view .back-btn');
    const nucleusContainer = document.getElementById('nucleus-container');
    const addProtonBtn = document.querySelector('.add-proton-btn');
    const addNeutronBtn = document.querySelector('.add-neutron-btn');
    const addElectronBtn = document.querySelector('.add-electron-btn');
    ...
    const elementIdentification = document.getElementById('element-identification');
    ...
    if (backBtn) {
        backBtn.addEventListener('click', function() {
            document.getElementById('atom-builder-view').style.display = 'none';
            resetAtomBuilder();
        });
    }
    ...
    // Add proton
    if (addProtonBtn) {
        addProtonBtn.addEventListener('click', function() {
            if (protons + neutrons < 20) { // Limit to prevent overflow
                const proton = document.createElement('div');
                proton.className = 'builder-proton';
                nucleusContainer.appendChild(proton);
                protons++;
                updateCounts();
                identifyElement();
            }
    ...
    // Add neutron
    if (addNeutronBtn) {
        addNeutronBtn.addEventListener('click', function() {
            if (protons + neutrons < 20) { // Limit to prevent overflow
                const neutron = document.createElement('div');
                neutron.className = 'builder-neutron';
                nucleusContainer.appendChild(neutron);
                neutrons++;
                updateCounts();
            }
        });
    ...
    // Add electron
    if (addElectronBtn) {
        addElectronBtn.addEventListener('click', function() {
            if (electrons < 8) { // Limit to first shell
                const electron = document.createElement('div');
                electron.className = 'builder-electron';
                
                // Position on appropriate orbit
                let orbit, angle, radius;
                if (electrons < 2) {
                    orbit = document.querySelector('.orbit-1');
                    radius = 120; // Half of orbit-1 width
                } else {
                    orbit = document.querySelector('.orbit-2');
                    radius = 160; // Half of orbit-2 width
                }
                
                // Calculate position
                angle = (electrons * 45) % 360; // Distribute evenly
    ...
    function identifyElement() {
        ...
        if (elements[protons]) {
            const elementName = getTranslatedElementName(elements[protons], currentLang);
            const message = translations[currentLang].element_identified.replace('{element}', elementName);
            elementIdentification.textContent = message;
            elementIdentification.style.display = 'block';
        } else {
            elementIdentification.textContent = '';
            elementIdentification.style.display = 'none';
        }
    }
    
    // Reset the atom builder
    function resetAtomBuilder() {
        if (nucleusContainer) {
            nucleusContainer.innerHTML = '';
            document.querySelectorAll('.builder-electron').forEach(el => el.remove());
            protons = 0;
            neutrons = 0;
            electrons = 0;
            updateCounts();
    ...
    // Get current language (integrate with your existing language system)
    function getCurrentLanguage() {
        // Replace with your actual method of getting the current language
        return document.documentElement.lang || 'en';
    }
    
    // Get translated element name
    function getTranslatedElementName(element, lang) {
        ...
        return elementTranslations[lang]?.[element] || element;
    }
});

// Add translation entries for atom builder (integrate with your translation system)
const translations = {
    en: {
        atom_builder_title: 'Atom Builder',
        atom_builder_desc: 'Build your own atoms by adding protons, neutrons, and electrons!',
        add_proton: 'Add Proton',
        add_neutron: 'Add Neutron',
        add_electron: 'Add Electron',
        protons: 'Protons',
        neutrons: 'Neutrons',
        electrons: 'Electrons',
        element_identified: "You've built {element}!"
    },
    es: {
        atom_builder_title: 'Constructor de Átomos',
        atom_builder_desc: '¡Construye tus propios átomos añadiendo protones, neutrones y electrones!',
        add_proton: 'Añadir Protón',
        add_neutron: 'Añadir Neutrón',
        add_electron: 'Añadir Electrón',
        protons: 'Protones',
        neutrons: 'Neutrones',
        electrons: 'Electrones',
        element_identified: "¡Has construido {element}!"
    },
    bs: {
        atom_builder_title: 'Graditelj Atoma',
        atom_builder_desc: 'Izgradite svoje atome dodavanjem protona, neutrona i elektrona!',
        add_proton: 'Dodaj Proton',
        add_neutron: 'Dodaj Neutron',
        add_electron: 'Dodaj Elektron',
        protons: 'Protoni',
        neutrons: 'Neutroni',
        electrons: 'Elektroni',
        element_identified: "Izgradili ste {element}!"
    }
};

// Make sure to integrate this with your existing translation system
    ...
    const translations = {
        ...
        bs: {
            ...
            // List items (Generated Translations)
            "electron-info_li_0": "Elektroni su fundamentalne čestice koje kruže oko jezgra atoma.",
        ...
        tr: {
            ...
            // List items (Generated Translations)
            "electron-info_li_0": "Elektronlar, bir atomun çekirdeği etrafında dönen temel parçacıklardır.",
